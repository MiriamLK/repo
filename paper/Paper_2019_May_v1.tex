...
\title{Gender Segregation in Occupations: Empirical Evidence from a Matching Model with Transfers}
...
\abstract{Women have entered the labor force in increasing numbers but remain concentrated in certain occupations. I examine whether occupations remain segregated because workers care about gender when making occupation choices. I build a model of labor supply and demand where firms care about hiring men or women and wages, and workers care about wages, and the number of women in occupations in addition to the occupations themselves. Using a Bartik instrumental variables strategy, I find that women prefer to enter into female-dominated occupations, but men show no evidence of gender preference. In a world without women's gender preference, occupations would be 50\% less segregated and the gender wage gap 18\% smaller. However, women's preference for working with women would have to be almost twice as strong for past segregation to affect current segregation.}
...
Occupation gender segregation is a persistent feature of the labor market that continues to contribute substantially to the gender wage gap, especially as the education gap between men and women has closed and even reversed \cite{Blau2006, Kahn2016a}. Furthermore, convergence in the occupation choices of men and women has been slowing over time \cite{Blau2013}.  In recent literature, many factors have been identified as contributing causes of segregation, including preferences over amenities (\citeA{Olivieri2014a}, \citeA{Wiswall2013})\footnote{For overview see \citeA{ Bertrand2011} and \citeA{Cortes2017a}.}, productivity and skill differences (\citeA{Baker2015}), and occupational and educational barriers (\citeA{Hsieh2016}). In this paper I study one possible cause of segregation: preferences over the number of men and women in the occupation.
...
%the historical composition of a given occupation will determine whether it is dominated by one group or another.

%I refer to a stable gender composition, or a gender composition that can only be changed by changing supply or demand, as an equilibrium. Therefore I refer to the mechanism of tipping between two possible stable gender compositions as tipping caused by ``multiple equilibria". 



%Studying multiple equilibria in the labor market is particularly interesting because unlike in school or neighborhood segregation, wages can theoretically adjust to compensate groups for their preferences.

% add a paragraph on the role of wages????
...
%%%%%When gender preference is strong enough that an occupation can only be male or female, I say that the occupation has a ``male equilibrium" and a ``female equilibrium", or that the occupation has ``multiple equilibria" in the fraction female. Tipping caused by moving between multiple equilibria is of particular interest because it means that certain shocks to gender composition could lead to persistent and possibly welfare enhancing changes in the sorting pattern, even holding fixed underlying labor supply and demand. For example, if STEM or nursing are occupations that have multiple equilibria, then temporarily encouraging more women to enter STEM, or men to enter nursing, could lead to long run changes due to movement from a male-dominated equilibrium to a female-dominated equilibrium or vice versa.\footnote{Multiple equilibria is also the mechanism that \citeA{Pan2010} puts forth to explain tipping in occupations. I illustrate the \citeA{Pan2010} model, based on the social interactions model of \citeA{Schelling1971}, in Appendix \ref{sec.Pan}.}
...
%A one-sided model ignoring the firm side would not only bias estimates of worker preferences, it would also bias estimates of the location of tipping points and multiple equilibria. 
...
% Because segregation emerges from both sides of the market, and workers and firms are optimizing and connected through equilibrium wages,
%  Therefore I estimate a structural model in order to predict past and future segregation patterns including multiple equilibria.
...
%My setting is different because of wages?
